...
const
  HCIDEVUP* = IOW(ord('H'), 201, cint)
  HCIDEVDOWN* = IOW(ord('H'), 202, cint)
  HCIDEVRESET* = IOW(ord('H'), 203, cint)
  HCIDEVRESTAT* = IOW(ord('H'), 204, cint)
  HCIGETDEVLIST* = IOR(ord('H'), 210, cint)
  HCIGETDEVINFO* = IOR(ord('H'), 211, cint)
  HCIGETCONNLIST* = IOR(ord('H'), 212, cint)
  HCIGETCONNINFO* = IOR(ord('H'), 213, cint)
  HCIGETAUTHINFO* = IOR(ord('H'), 215, cint)
  HCISETRAW* = IOW(ord('H'), 220, cint)
  HCISETSCAN* = IOW(ord('H'), 221, cint)
  HCISETAUTH* = IOW(ord('H'), 222, cint)
  HCISETENCRYPT* = IOW(ord('H'), 223, cint)
  HCISETPTYPE* = IOW(ord('H'), 224, cint)
  HCISETLINKPOL* = IOW(ord('H'), 225, cint)
  HCISETLINKMODE* = IOW(ord('H'), 226, cint)
  HCISETACLMTU* = IOW(ord('H'), 227, cint)
  HCISETSCOMTU* = IOW(ord('H'), 228, cint)
  HCIBLOCKADDR* = IOW(ord('H'), 230, cint)
  HCIUNBLOCKADDR* = IOW(ord('H'), 231, cint)
  HCIINQUIRY* = IOR(ord('H'), 240, cint)
